...
public interface EmployeeRepository extends JpaRepository<Employee, Long> {
    ...
    @Query(value = "select * from employee where salary = (select  max (salary) from employee) limit 1", nativeQuery
            = true)
    Employee findOnlyOneGuyWithHighestSalary();

    @Query(value =  "select * from employee where salary between :from and :to", nativeQuery
            = true)
